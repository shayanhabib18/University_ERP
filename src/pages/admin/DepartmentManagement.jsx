...
import { Eye, Edit, Trash2, Plus, ChevronDown, ChevronUp, Save, X } from "lucide-react";
...
const DepartmentManagement = () => {
      ...
      semesters: [
        {
          number: 1,
          courses: [
            { id: 1, name: "Introduction to Programming", code: "CS101", preReq: "None" },
            { id: 2, name: "Discrete Mathematics", code: "MATH101", preReq: "None" }
          ]
        },
        {
          number: 2,
      ...
      semesters: [
        {
          number: 1,
          courses: [
            { id: 4, name: "Principles of Management", code: "BBA101", preReq: "None" }
          ]
        }
      ]
    }
  ...
  const handleAddCourse = () => {
    if (!newCourse.semester || !newCourse.name.trim() || !newCourse.code.trim()) {
    ...
    const updatedDepartments = departments.map(dept => {
      if (dept.id === selectedDept.id) {
        ...
        const semesterExists = dept.semesters.some(s => s.number === semesterNum);
    ...
    setDepartments(updatedDepartments);
    setSelectedDept(updatedDepartments.find(dept => dept.id === selectedDept.id));
    setNewCourse({
      semester: "",
      name: "",
      code: "",
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow max-w-7xl mx-auto">
      <h1 className="text-2xl font-bold mb-6 text-indigo-700">
        {selectedDept ? `${selectedDept.name} (${selectedDept.code})` : "Department Management"}
      </h1>
      ...
      {!selectedDept ? (
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="text-sm font-medium text-gray-500">Total Semesters</h3>
              <p className="text-2xl font-semibold">{selectedDept.semesters.length}</p>
            </div>
          </div>
          ...
          {selectedDept.semesters.length === 0 ? (
            ...
            selectedDept.semesters.map(semester => (
              <div key={semester.number} className="mb-4 border border-gray-300 rounded-lg">
                <div
                ...
                >
                  ...
                  {expandedSemesters[semester.number] ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                </div>
                        ...
                        <tr key={course.id} className="border-t border-gray-200">
                          ...
                          <td className="px-4 py-2 flex gap-3">
                            <button
                              ...
                              title="Edit Course"
                              onClick={() => setEditingCourse({ ...course, semesterNumber: semester.number })}
                            >
                              <Edit size={16} />
                            </button>
                            <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          <div className="bg-white rounded-lg p-6 w-96 shadow-lg">
            ...
            <input
              ...
              onChange={(e) => setEditingCourse({ ...editingCourse, preReq: e.target.value })}
              className="border border-gray-300 rounded px-3 py-2 w-full mb-4 focus:outline-indigo-600"
            />
