...
export default function FacultyForgotPassword() {
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      const resp = await fetch("http://localhost:5000/faculties/forgot-password", {
        method: "POST",
      ...
      setEmail("");
      setTimeout(() => navigate("/login/faculty"), 3000);
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-white p-4">
      <div className="bg-white p-8 rounded-xl shadow-md w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <h2 className="text-3xl font-bold text-blue-700">Faculty Forgot Password</h2>
          <p className="text-gray-600 mt-2 text-sm">Enter your faculty email to receive a password reset link</p>
        </div>
        ...
        <form onSubmit={handleForgotPassword} className="space-y-4">
          <div>
            <label className="block text-gray-700 font-medium mb-2">Faculty Email Address</label>
            <input
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
              placeholder="Enter your registered faculty email"
              required
              disabled={loading}
            />
          </div>
